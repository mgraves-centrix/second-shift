...
## Purpose
One place to ask what the system resolved and where each value came from —
environment, a named file, or a default — so a wrong setting on the always-on
machine can be corrected without reading Python, and so a configuration file
that exists but cannot be parsed is never mistaken for one that is absent.
