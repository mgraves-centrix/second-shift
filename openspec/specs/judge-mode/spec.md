...
## Purpose
The product, demonstrable in one sitting, with none of the subject's real
data in it. A judge has minutes and no context: they will not read this
repository, install anything, or wait for a night to run. So the first screen
explains before it asks, the night it shows is a recorded one — no reasoner is
bound on this profile and running one would write the prompt back as every
artifact — and every row it holds is marked synthetic, enforced at the
deployment's edge rather than documented. Principle 5 in one deployment: the
same code as the personal instance, separated only by compute profile.
