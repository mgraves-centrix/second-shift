...
- [x] 1.1 Add `numpy` to `apps/api/pyproject.toml` and verify
  `python -c "import numpy"` succeeds inside the venv.
- [x] 1.2 Add an `[embedder]` table to `config/models.toml` — host, port
  ...
  parses it.
- [x] 1.3 Add `_embedder_config()` to `config.py`, mirroring `_local_config()`,
...
- [x] 2.1 Implement `VllmEmbedder(Embedder)` in `providers/vllm_embedder.py`
...
- [x] 2.2 Write `LocalEmbedderNotConfigured`, raised when `[embedder]` is
...
- [x] 2.3 Narrow the registry: bind `VllmEmbedder` where configured,
...
- [x] 2.4 Write `deploy/spark/second-shift-embedder.service`, mirroring
...
- [x] 3.1 Create `apps/api/secondshift/retrieval/` with a `ContextPiece`
...
- [x] 3.2 Implement `RetrievalIndex.rebuild()` — reads every `entries` row via
...
- [x] 3.3 Implement `RetrievalIndex.search(query_vector, k)` — exact cosine
  ...
  on repeated runs.
- [x] 3.4 Implement `assemble_context(query, *, policy, max_pieces)` — embeds
...
- [x] 3.5 Handle an embedding or search failure by letting it propagate as a
...
- [x] 4.1 **The airlock test.** Assemble context for a prompt destined for a
  ...
  passes.
- [x] 4.2 Assert retrieval on the `cloud` profile constructs no remote
...
- [x] 4.3 Assert rebuilding the index after deletion returns identical search
  results to before deletion, over a fixed corpus and a fixed query.
- [x] 4.4 Assert reading the brain at two different pinned commits returns
