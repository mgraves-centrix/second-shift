...
- [ ] 1.1 `create_backup` refuses a destination on the same device as the
...
- [ ] 1.2 A test for the refusal, and one for the flag. Both construct the
      case rather than describing it.
- [ ] 1.3 `doctor` reports a newest backup that sits on the database's own
      device. Weeks of successful local backups is a state to be told about,
      not to discover.

## 2. The unit

- [ ] 2.1 `deploy/spark/second-shift-backup.user.service` — `oneshot`,
      `After=second-shift-night.service`, destination from
      `%h/.config/second-shift/backup.env` with no leading `-` so a missing
      file fails loudly.
- [ ] 2.2 `Wants=second-shift-backup.service` on the night's unit, which is
      what makes it run after a failed night too.
- [ ] 2.3 Extend `test_deploy_units.py`: the ordering, the coupling, the
...
- [ ] 3.1 Annotate the marker in `2026-09-21-add-operations`'s archived design
...
- [ ] 3.2 `docs/operations/RECOVERY.md`: the schedule, and the rehearsal's flag.
- [ ] 3.3 `docs/SPEC_ROADMAP.md`: the ledger row for the schedule now says what
